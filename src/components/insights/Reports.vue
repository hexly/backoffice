      ...
      <template>
        ...
        >
          ...
          <template v-slot:item.rank="{ item }">
            <v-chip
              :color="item.rank > 5 ? '#a1213b' : 'gray'"
              :class="{'white--text': item.rank > 5}"
            >Rank {{item.rank}}</v-chip>
          </template>
          <template v-slot:item.recognizedRank="{ item }">
            <v-chip
              :color="item.current.recognizedRank > 5 ? '#a1213b' : 'gray'"
              :class="{'white--text': item.current.recognizedRank > 5}"
            >Rank {{item.current.recognizedRank}}</v-chip>
          </template>
          <template v-slot:item.memberPath="{ item }">
            <v-layout
              class="generation-badge-container"
              align-center
              row
              wrap
            >
              <template v-for="(parent, index) in item.ancestors">
                <v-tooltip
                  v-if="index > 0 && (index + 1) !== item.ancestors.length"
                  top
                  slot="append"
                ...
                >
                  <template v-slot:activator="{ on }">
                    <v-avatar
                      class="generation-avatar ma-0 elevation-3"
                      size="24px"
                      v-on="on"
                    >
                      <img
          ...
          <template v-slot:item.cpsv="{ item }">
            <v-chip
              :color="item.next.stats.lifetimeTotalPoints.satisfied ? '#4CAF50' : '#EF5350'"
              :class="{'white--text': true}"
            >{{ Math.floor(item.cpsv) }}</v-chip>
          </template>
          <template v-slot:item.stats="{ item }">
            <v-row
              class="the-grid"
              v-if="$tenantInfo.id == 1010"
            >
              <v-col
                v-if="$tenantInfo.statMapping.personalTotalAmount"
                cols="6"
                class="bottom-border right-border grid-cell"
                :class="{'satisfied': item.next.stats.personalTotalAmount.satisfied}"
              >
              ...
              <v-col
                v-if="$tenantInfo.statMapping.personalTotalPoints"
                cols="6"
                class="bottom-border right-border grid-cell"
                :class="{'satisfied': item.next.stats.personalTotalPoints.satisfied}"
              >
                <h6>{{$tenantInfo.statMapping.personalTotalPoints.title}}</h6>
                <div class="text-center">{{Math.floor(item.psv)}}</div>
              </v-col>
              <v-col
                v-if="$tenantInfo.statMapping.groupPoints"
                cols="6"
                class="bottom-border grid-cell"
                :class="{'satisfied': item.next.stats.groupPoints.satisfied}"
              >
                <h6>{{$tenantInfo.statMapping.groupPoints.title}}</h6>
                <div class="text-center">{{Math.floor(item.gsv)}}</div>
              </v-col>
              <v-col
                v-if="$tenantInfo.statMapping.downlinePoints"
                cols="6"
                class="right-border grid-cell"
                :class="{'satisfied': item.next.stats.downlinePoints.satisfied}"
              >
                <h6>{{$tenantInfo.statMapping.downlinePoints.title}}</h6>
                <div class="text-center">{{Math.floor(item.dsv)}}</div>
              </v-col>
              <v-col
                v-if="$tenantInfo.statMapping.activeLeg"
                cols="6"
                class="grid-cell"
                :class="{'satisfied': item.next.stats.activeLeg.satisfied}"
              >
            ...
            >
              ...
              <v-col
                v-if="$tenantInfo.statMapping.personalCommissionablePoints"
                cols="6"
                class="bottom-border grid-cell"
                :class="{'satisfied': item.next.stats.personalCommissionablePoints.satisfied}"
              >
        ...
        <v-expansion-panels>
          ...
          <v-expansion-panel v-if="selectedPeriod.metadata && selectedPeriod.metadata.version === 2 && engineStats.metadata">
            ...
            <v-expansion-panel-content>
              <v-radio-group v-model="sortBy">
                <v-radio
                  v-for="(value, key) in newSortByOptions"
                  :label="value"
                  :value="key"
                  :key="key"
                ></v-radio>
          ...
          <v-expansion-panel v-else>
            ...
            <v-expansion-panel-content>
              <v-radio-group v-model="sortBy">
                <v-radio
                  v-for="(value, key) in sortByOptions"
                  :label="value"
                  :value="key"
                  :key="key"
                ></v-radio>
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-content>
              <v-radio-group v-model="orderBy">
                <v-radio
                  v-for="(value, key) in orderByOptions"
                  :label="value"
                  :value="key"
                  :key="key"
                ></v-radio>
